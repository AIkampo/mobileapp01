...
class ChartLinePainter extends BasePainter {
        ...
        if (item.chartBeans != null && item.chartBeans.isNotEmpty) {
          if (item.isDrawX) {
            ...
            for (var i = 0; i < item.chartBeans.length; i++) {
              ...
              // tempArr2.add(DialStyleX(
              //     title: (item.chartBeans[i].y/100).toStringAsFixed(2),
              //     titleStyle: item.xTitleStyle,
              //     positionRetioy: 0.2,
              //     positionRetioy: item.chartBeans.length == 1
                  //     ? 0.0
          ...
          for (var i = 0; i < length; i++) {
            // Fimber.i('$TAG _initPath: i = $i, _startX = ${item.chartBeans[i].x}, y = ${item.chartBeans[i].y}');
            currentX = _startX + W * i;
            currentY = (_startY -
                (item.chartBeans[i].y / baseBean.yMax) * _fixedHeight);
            // Fimber.i('$TAG _initPath: i = $i, ${item.chartBeans[i].y}, ${baseBean.yMax}, $_fixedHeight');
            // Fimber.i('$TAG _initPath: i = $i, currentX = $currentX, currentY = $currentY');
            if (i == 0) {
              preX = currentX;
              preY = currentY;
            }
  ...
  void paintText(Canvas canvas, String _text, Rect _rect) {
    double _width = 30;
    double _height = _rect.bottom - _rect.top;
    ...
    final xCenter = _rect.left - 8;
    final yCenter = _rect.top + _height / 2 - 8;
    final offset = Offset(xCenter, yCenter);
    // Fimber.i('$TAG paintText: xCenter = $xCenter, yCenter = $yCenter, width = $_width, height = $_height');
    textPainter.paint(canvas, offset);
  }

  /// 曲线或折线
  void _drawLine(Canvas canvas) {
    // Fimber.i("$TAG _drawLine:");
    _lineCanvasModels.forEach((element) {
      ...
      if (element.points != null) {
        ...
        element.points.forEach((pointElement) {
          i++;
          int _length = element.points.length;
          // Fimber.i("$TAG _drawLine: _length = $_length, i = $i");
          ...
          var rect2 = Rect.fromLTRB(
              pointElement.x - element.pointRadius,
              pointElement.y - element.pointRadius - 10,
              pointElement.x + element.pointRadius,
              pointElement.y + element.pointRadius - 10);
          ...
          canvas.drawRRect(
              RRect.fromRectAndCorners(rect,
                  ...
                  bottomRight: Radius.circular(borderRaius)),
              pointPaint);
          Size _size = new Size(rect2.right-rect2.left, rect2.bottom-rect2.top);
          paintText(canvas, chartBeanSystems[0].chartBeans[i+1].y.toInt().toString(), rect2);
        });
      }
